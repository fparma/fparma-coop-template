...
Description:
	...
	Does not delete units who are dead when mission starts
	Can also use "this setVariable ["fp_noDelete", true]"
...
_isHidden = compileFinal "
	private [""_c""];
	_c = FALSE;
	if (({(((_this select 0) distance _x) < (_this select 1))} count (_this select 2)) isEqualTo 0 && {(_this select 0) distance [0,0,0] > 10}) then {
		_c = TRUE;
	};
	_c;
";
...
QS_deleteManagerPublic = TRUE;						// To terminate script via debug console
...
_playerThreshold = 15;							// How many players before accelerated cycle kicks in?

_deadMenLimit = 20;								// Bodies. Set -1 to disable.
...
_deadVehicleDist = 800;						// Distance (meters) from players that destroyed vehicles are not deleted if below max.

_craterLimit = 10;								// Craters. Set -1 to disable.
_craterDistCheck = TRUE;						// TRUE to delete any craters that are far from players.
_craterDist = 600;								// Distance (meters) from players that craters are not deleted if below max.

_weaponHolderLimit = 12;						// Weapon Holders. Set -1 to disable.
_weaponHolderDistCheck = TRUE;					// TRUE to delete any weapon holders that are far from players.
_weaponHolderDist = 50;							// Distance (meters) from players that ground garbage is not deleted if below max.
...
_minesDist = 1500;								// Distance (meters) from players that land mines are not deleted if below max.
...
// Do not delete units who are dead when mission starts
// Can also use "this setVariable ["fp_noDelete", true]"
_startDead = allDead;

sleep 14;

while {QS_deleteManagerPublic} do {

	//================================= DEAD MEN
	if (!(_deadMenLimit isEqualTo -1)) then {
		if ((count allDeadMen) > _deadMenLimit) then {
			// todo- maybe infiinite loop?
			while {(((count (allDeadMen-[_startDead])) - _deadMenLimit) > 0)} do {
			...
			if (_deadMenDistCheck) then {
				{
					if ([_x,_deadMenDist,(playableUnits + switchableUnits)] call _isHidden && {(!(_x in _startDead))} && {(!(_x getVariable ["fp_noDelete", false]))}) then {
						detach _x;
						deleteVehicle _x;
					};
				} count allDeadMen;
	...
	if (!(_deadVehiclesLimit isEqualTo -1)) then {
		if ((count (allDead - allDeadMen)) > _deadVehiclesLimit) then {
			...
			if (_deadVehicleDistCheck) then {
				{
					if ([_x,_deadVehicleDist,(playableUnits + switchableUnits)] call _isHidden && {(!(_x in _startDead))} && {!(_x getVariable ["fp_noDelete", false])}) then {
						deleteVehicle _x;
					};
