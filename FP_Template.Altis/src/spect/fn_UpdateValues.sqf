...
while {true} do
{
	...
	{
		ctrlSetText [1000,format ["Spectating: %1", if (name f_cam_curTarget == 'Error: No unit') then {'Your dead body'} else{name f_cam_curTarget}]];
	}
	else
	{
		ctrlSetText [1000,format ["Spectating: %1", "Unknown"]];
	};
	_listBox =  2100;
	//lbClear _listBox;
